...
() {
  ...
  local -a codenames
  zstyle -a ":plugin:zuid" codenames codenames || codenames=(
    atlantis echelon quantum ion proxima polaris solar momentum hyper gloom velocity future enigma andromeda
    saturn jupiter aslan commodore falcon persepolis dharma samsara prodigy ethereal epiphany aurora oblivion
  )
...
() {
  ...
  # with Zsh 5.3, 5.3.1 - zsystem's obtained file descriptors cannot be written to
  [[ "$ZUID_FD" -ne "0" ]] && { echo "$$" >&${ZUID_FD} } 2>/dev/null

  (( ZUID_ID )) && ZUID_CODENAME="${${(@s,:,)ZUID_CONFIG[codenames]}[$ZUID_ID]}"
}

# Not called ideally at say SIGTERM, but at least when "exit" is enterred
